...
function initializeSettings() {
    ...
    // Load weather settings if on settings page
    loadWeatherSettings();
}
...
function checkOnlineStatus() {
    ...
    .catch(error => {
        ...
        console.debug('Status check failed (normal):', error.message);
    });
}
...
function handlePromiseError(event) {
    ...
    // Temporarily disable all error popups to stop interruptions
    // We'll just log everything to console for debugging
    
    // TODO: Re-enable with better filtering once we identify the root cause
}
...
// Global weather settings object
let weatherSettings = {};
...
async function loadWeatherSettings() {
    try {
        ...
        if (result.success) {
            weatherSettings = result.data;
            populateWeatherSettings();
        } else {
            console.error('Failed to load weather settings:', result.error);
        }
    } catch (error) {
...
/**
 * Populate weather settings form with current values
 */
function populateWeatherSettings() {
    // Temperature unit
    const tempUnit = document.getElementById('temperature-unit');
    if (tempUnit) {
        tempUnit.value = weatherSettings.temperature_unit || 'F';
    }
    ...
    const autoRadio = document.querySelector('input[name="location-mode"][value="auto"]');
    const customRadio = document.querySelector('input[name="location-mode"][value="custom"]');
    
    if (autoRadio && customRadio) {
        autoRadio.checked = !isCustomEnabled;
        customRadio.checked = isCustomEnabled;
        toggleLocationMode(isCustomEnabled ? 'custom' : 'auto');
    }
    
    // Custom location fields
    if (isCustomEnabled) {
        const cityField = document.getElementById('custom-city');
        const countryField = document.getElementById('custom-country');
        const latField = document.getElementById('custom-latitude');
        const lonField = document.getElementById('custom-longitude');
    ...
    const enableSecondCheckbox = document.getElementById('enable-second-location');
    ...
    if (secondEnabled) {
        const nameField = document.getElementById('second-location-name');
        const latField = document.getElementById('second-latitude');
        const lonField = document.getElementById('second-longitude');
    ...
    const refreshField = document.getElementById('weather-refresh');
    if (refreshField) {
...
/**
 * Update a single weather setting
 */
async function updateWeatherSetting(key, value) {
    try {
        ...
        if (result.success) {
            weatherSettings[key] = value;
            showNotification('Weather settings updated successfully', 'success');
        } else {
            showNotification('Failed to update weather settings: ' + result.error, 'error');
        }
    } catch (error) {
        console.error('Error updating weather setting:', error);
        showNotification('Error updating weather settings', 'error');
    }
}

/**
 * Toggle location mode between auto and custom
 */
function toggleLocationMode(mode) {
    const customSettings = document.getElementById('custom-location-settings');
...
async function updateCustomLocation() {
    ...
    const latitude = parseFloat(document.getElementById('custom-latitude').value);
    const longitude = parseFloat(document.getElementById('custom-longitude').value);
    
    // Validate required fields
    if ((!city || !country) && (!latitude || !longitude)) {
        showNotification('Please enter either city/country or latitude/longitude', 'error');
        return;
    }
...
/**
 * Toggle second location display
 */
function toggleSecondLocation() {
    const enabled = document.getElementById('enable-second-location').checked;
    const settings = document.getElementById('second-location-settings');
    ...
    const currentSecondLocation = weatherSettings.second_location || {};
    currentSecondLocation.enabled = enabled;
    updateWeatherSetting('second_location', currentSecondLocation);
}

/**
 * Update second location settings
 */
async function updateSecondLocation() {
    const name = document.getElementById('second-location-name').value;
    const latitude = parseFloat(document.getElementById('second-latitude').value);
    const longitude = parseFloat(document.getElementById('second-longitude').value);
    
    if (!name || !latitude || !longitude) {
        showNotification('Please fill in all second location fields', 'error');
        return;
    }
...
// Weather settings functions
window.loadWeatherSettings = loadWeatherSettings;
window.updateWeatherSetting = updateWeatherSetting;
window.toggleLocationMode = toggleLocationMode;
window.updateCustomLocation = updateCustomLocation;
window.toggleSecondLocation = toggleSecondLocation;
window.updateSecondLocation = updateSecondLocation;